...
import { ConfidenceLevel, SqlProblem } from "@/types";

export default function SqlProblemPage() {
  const { id } = useParams();
  const router = useRouter();
  const { sqlProblems, markSqlSolved, bookmarkSql, skipSql, needSqlReview, updateSqlNotes, getNextSqlProblem } = useSqlStore();
  const { problemOrder } = useAppStore();
  const problem = sqlProblems.find(p => p.id === Number(id));
  ...
  const goToNext = () => {
    const next = getNextSqlProblem(problemOrder);
    if (next) {
      router.push(`/sql/problem/${next.id}`);
    } else {
      router.push('/sql');
    }
  };
  ...
  const submitSolved = (confidence: ConfidenceLevel) => {
    if (problem) {
      markSqlSolved(problem.id, seconds, confidence, notes);
      goToNext();
    }
  };
  ...
  if (showConfidence) {
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] animate-in fade-in zoom-in duration-300">
        <h2 className="text-3xl font-bold mb-2">SQL Problem Solved! 🎉</h2>
      ...
      <div className="flex justify-between items-start mb-6">
        <div>
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-xs px-2.5 py-0.5 rounded-full font-medium border border-blue-200 bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:border-blue-500/20 dark:text-blue-400">
              {problem.source}
            </span>
          </div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-6 h-full">
          ...
          <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl flex-1 flex flex-col overflow-hidden">
            <div className="border-b border-gray-200 dark:border-zinc-800 px-4 py-3 flex items-center justify-between pb-2">
              <h3 className="font-semibold">Query Notes / Learnings</h3>
              <button 
                onClick={() => updateSqlNotes(problem.id, notes)}
            ...
            <textarea 
              ...
              placeholder="Record any clever queries, edge cases, or optimizations here..."
          ...
          <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl flex flex-col overflow-hidden">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium">Open in {problem.source}</span>
              </div>
            </a>
            <a 
              href={`https://www.youtube.com/results?search_query=sql+${encodeURIComponent(problem.title)}`} 
              target="_blank" 
              rel="noopener noreferrer"
